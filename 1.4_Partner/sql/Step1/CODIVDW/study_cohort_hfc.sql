...
INTO #study_cohort
 ...
 WHERE PERSON_ID COLLATE SQL_Latin1_General_CP1_CS_AS IN (
	SELECT 
		@PERSON_ID_PATID COLLATE SQL_Latin1_General_CP1_CS_AS
	FROM 
		@SCHEMA.@SESSION s
		 WHERE DATEPART(YEAR, session_date) = 2017
		 --AND programid IN (SELECT programid from #study_programs)
	EXCEPT 
	SELECT @PERSON_ID_PATID COLLATE SQL_Latin1_General_CP1_CS_AS
